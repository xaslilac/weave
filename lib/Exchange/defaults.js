...
Object.assign( weave.Exchange.prototype, {
  print( error, manifest ) {
    ...
    if ( error ) {
      if ( error instanceof weave.HttpError ) {
        if ( manifest.isFile() ) return this.printFile( error, manifest )
        else return this.printError( error, manifest )
      }

      this.printError( new weave.HttpError( 500 ), manifest, this )
      throw garden.typeerror( 'Error argument was not a weave.HttpError!', error )
    }
  ...
  printFile( error, manifest ) {
    ...
    fs.open( manifest.path, 'r', ( ferror, fd ) => {
      if ( ferror ) return this.generateErrorPage( new weave.HttpError( 500, ferror ) )
      ...
      if ( rangeMatch ) {
        ...
        if ( bufferState.readPosition > bufferState.readEnd
          || bufferState.readEnd > manifest.stats.size ) return this.generateErrorPage( 416 )
        ...
        this.head( error ? error.statusCode : 200, {
          ...
          'Content-Type': this.behavior( `mimeTypes ${extname}` ) || defaultDictionary[ extname ]
        })
      }
  ...
  printDirectory( error, manifest ) {
    fs.readdir( manifest.path, ( derror, files ) => {
      if ( derror ) return this.generateErrorPage( new weave.HttpError( 500, derror ) )
