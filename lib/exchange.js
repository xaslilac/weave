...
weave.Exchange = class Exchange extends events.EventEmitter {
    ...
    this.garden = weave.createGarden(
      `weave.Exchange(${this.url.original}{${this.UUID.slice(0, 13)}})`,
      { color: 210 })
    this.garden.time( 'response' )
	  ...
	  if ( weave.hosts[ this.host ] ) {
	    ...
	    this.app = weave.hosts[ weave.cache.hostMatches[ this.host ] ]
	      ...
	      Object.keys( weave.hosts ).filter( host => /\*/.test( host ) ), this.host
      ).toString()
	  ...
	  if ( this.configuration ) {
		...
		if ( typeof domain === 'string' && domain !== this.url.hostname )
			return this.redirect( url.format( Object.assign( this.url, {
        host: `${domain}:${this.url.port}`, pathname: this.url.original } ) ))

		// Enforce secure connections
		let force = this.behavior( 'secure' )
		if ( force && !secure )
      return this.redirect( url.format( Object.assign( this.url, { protocol: 'https:',
      host: `${this.url.hostname}:443`, pathname: this.url.original } ) ))
