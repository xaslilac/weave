...
weave.App = class App extends EventEmitter {
  ...
  intercept( mountUrl, handle ) {
    if ( typeof mountUrl !== 'string' ) throw garden.typeerror( 'Argument mountUrl must be a string!' )
    if ( !path.isAbsolute( mountUrl ) ) throw garden.error( 'Argument mountUrl must be absolute!' )
    if ( handle == null ) throw garden.typeerror( 'No handle given to intercept ${ mountUrl }!' )
    if ( typeof handle.createIntercept === 'function' ) handle = handle.createIntercept()
    ...
    // current mounts. If it is, we should ask the user to nest the mounts.
    let overlap = Object.keys( this.mounts ).some( mount => mount.startsWith( mountUrl + '/' ) || mountUrl.startsWith( mount + '/' ))
    if ( overlap ) return garden.error( `Intercepting in "${mountUrl}" would cause overlap with the existing intercept "${overlap}". Nesting should be used instead.` )
