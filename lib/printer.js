...
function printFileHead( error, manifest, exchange ) {
  // We will need to make our Content-Length reporting better eventually to
  // support preprocessing. It doesn't actually matter when you use chunked encoding,
  // and we really only use it to give meaningful download estimates for large files.
  // It'd still be nice to have it be accurate.
  ...
  exchange.status( error ? error.statusCode : 200 )
    ...
    .header( 'Content-Length', manifest.stats.size )
...
function printFile( error, manifest, exchange ) {
  ...
  fs.open( manifest.path, 'r', ( error, fd ) => {
    ...
    printFileHead( null, manifest, exchange )
...
function bufferFile( bufferState, exchange ) {
  let { fd, readBuffer, readPosition, remainder, readSize } = bufferState

  fs.read( fd, readBuffer, 0, Math.min( remainder, readSize ), readPosition, ( error, bytesRead ) => {
    if ( error ) return exchange.generateErrorPage( new weave.HttpError( 500, error ) )
    ...
    // Update our file positions.
    Object.assign( bufferState, {
      readPosition: readPosition + bytesRead,
      remainder: remainder - bytesRead
    })
