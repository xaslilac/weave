...
function printFile( error, manifest, exchange ) {
  ...
  fs.open( manifest.path, 'r', ( ferror, fd ) => {
    ...
    //
    // XXX: Something about this code breaks video streaming in Chrome. Everything
    // else works fine, but I don't want to commit these changes until that's resolved.
    //

    // let rangeMatch = /^bytes=([0-9]+)\-([0-9]*)$/i.exec( exchange.detail( 'range' ) )
    ...
    // exchange.status( error ? error.statusCode : (rangeMatch ? 206 : 200) )
      ...
      .header( 'Content-Type', exchange.behavior( `mimeTypes ${extname}` ) || defaultDictionary[ extname ] )

    // if ( rangeMatch ) {
    //   exchange.header( 'Content-Length', bufferState.readEnd - bufferState.readPosition )
    //   exchange.header( 'Content-Range', `bytes ${bufferState.readPosition}-${bufferState.readEnd}/${manifest.stats.size}` )
    // } else {
  ...
  fs.readdir( manifest.path, ( derror, files ) => {
    ...
    Promise.all( files.map( file => new Promise( ( fulfill, reject ) => {
      ...
      listing.forEach( ({ file, stats }) => {
        ...
        let target = isDir ? directoryList : fileList
        let tr = document.createElement( 'tr' )
        let name = document.createElement( 'td' )
        let a = document.createElement( 'a' )
        a.href = path.join( '/', exchange.requestUrl.pathname, file )
        a.innerHTML = isDir ? `/${file}/` : `/${file}`
        a.className = isDir ? 'directory' : 'file'
      ...
      directoryList.forEach( item => table.appendChild( item ) )
      fileList.forEach( item => table.appendChild( item ) )
