...
weave.Exchange = class Exchange extends events.EventEmitter {
    ...
    garden.time( this.requestUrl.pathname )
	  ...
	  if ( weave.hosts[ this.requestUrl.host ] ) {
	    this.app = weave.hosts[ this.requestUrl.host ]
	  } else if ( weave.cache.hostMatches[ this.requestUrl.host ] ) {
	    this.app = weave.hosts[ weave.cache.hostMatches[ this.requestUrl.host ] ]
	  } else {
			let appHostName = Spirit.bestMatch(
	      Object.keys( weave.hosts ).filter( host => /\*/.test( host ) ), this.requestUrl.host
      ).toString()
	    ...
	    // If there isn't a linked app then just end the connection.
	      ...
	      return request.connection.destroy()
			...
			weave.cache.hostMatches[ this.requestUrl.host ] = appHostName
