...
let weave = require( '..' )
let garden = require( 'gardens' ).createScope( 'weave.App::printer' )
...
Object.assign( weave.Exchange.prototype, {
  ...
  handleInterface() {
    ...
    this.relativeUrl.description = this.relativeUrl.pathname
    let manifest = new weave.Manifest( { url: this.relativeUrl, type: 'interface' } )
...
function printDirectory( error, manifest, exchange ) {
  fs.readdir( manifest.path, ( derror, files ) => {
    ...
    let document = weave.createPageFromTemplate( `Contents of ${exchange.requestUrl.pathname}` )
