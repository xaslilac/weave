...
module.exports = function router() {
  ...
  if ( this.isUpgrade ) {
    return this.generateErrorPage( 501, 'Cannot Upgrade' )
  }
  ...
  if ( !'get head post'.includes( this.method ) ) {
    return this.generateErrorPage( 405, `Only GET, HEAD, and POST requests are supported.` )
  }
  ...
  let print = more => {
    ...
    this.print( null, manifest.extend( more ) )
  }
  ...
  if ( typeof location !== 'string' ) throw garden.error( `No location set for ${this.requestUrl.pathname}! Cannot route!` )
      ...
      if ( notFound ) {
        ...
        if ( this.relativeUrl.depth === 0 && Array.isArray( extensions ) && this.relativeUrl.pathname.match( /[A-Za-z0-9\-_]$/ ) ) {
          Promise.all( extensions.map( extension =>
            new Promise( ( missing, found ) => {
              fs.stat( cursor + extension, ( error, extensionStats ) => {
                if ( error || !extensionStats.isFile() ) return missing()
                found({ path: cursor + extension, stats: extensionStats, type: 'file' })
              })
            })) ).then( reroute, print )
        } else reroute()
      } else if ( this.relativeUrl.depth === 0 && stats.isFile() ) {
        // If this is the exact url requested and it is a file, serve it.
        print({ path: cursor, stats, type: "file" })
        ...
        if ( this.behavior( 'urlCleaning' ) && this.relativeUrl.depth === 0 && !this.requestUrl.pathname.endsWith('/') ) {
            ...
            new Promise( ( missing, found ) => {
              if ( this.relativeUrl.depth <= indexes[ index ] ) {
                ...
                fs.stat( index, ( error, indexStats ) => {
                  if ( error || !indexStats.isFile() ) return missing()
                  found({ path: index, stats: indexStats, type: 'file' })
                })
              } else missing()
            })
          ...
          // promise that fulfills so that Promise.all will call our backup logic.
            ...
            if ( this.relativeUrl.depth === 0 && this.behavior( 'htmlDirectoryListings' ) ) {
              // print({ path: cursor, stats, type: 'directory' })
              this.printDirectory( manifest.extend({ path: cursor, stats, type: 'directory' }))
            ...
            && this.relativeUrl.depth === 1 && this.relativeUrl.suffix === 'directory.json' ) {
              // print({ path: cursor, stats, type: 'directory', contentType: 'application/json' })
              this.printDirectory( manifest.extend({ path: cursor, stats, type: 'directory', contentType: 'application/json' }))
            } else {
