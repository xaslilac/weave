...
function attachServer( port, security, callback ) {
  if ( typeof callback !== 'function' ) return garden.error( 'createServer requires a callback!' )
  if ( weave.servers[ port ] ) return
  ...
  server.on( 'listening', error => callback( error ) )
  server.listen( port, '::' )
...
weave.App = class App extends events.EventEmitter {
  ...
  link( host = 80 ) {
    // Connect all of our dots
    let { port } = mountApp( host, this )
    attachServer( port, null, error => this.emit( 'listening', error ) )
  ...
  secure( security, host = 443 ) {
    let { port } = mountApp( host, this )
    attachServer( port, security, error => this.emit( 'listening', error ) )
