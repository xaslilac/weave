...
weave.Exchange = class Exchange extends events.EventEmitter {
    ...
    garden.time( this.requestUrl.pathname )
	  ...
	  if ( this.app.cache.parentDirectories[ this.requestUrl.pathname ] ) {
	    this.app = this.app.cache.parentDirectories[ this.requestUrl.pathname ]
	  } else {
      Object.keys( this.app.mounts ).some( pathname => {
        ...
        && ( this.requestUrl.pathname === pathname || this.requestUrl.pathname.charAt( pathname.length ) === "/" ) ) {
          this.app = this.app.cache.parentDirectories[ this.requestUrl.pathname ] = this.app.mounts[ pathname ]
    ...
    // TODO: Change this to forbidden, to represent the 403 code
		if ( this.options.hasOwnProperty( 'access' ) ) {
			if ( this.options.access === false
			  || this.options.access[ this.relativeUrl.pathname ] === false )
  ...
  // Methods for getting information about the request
	...
	behavior( name ) {
	  ...
	  let nests = name.split(" ")
		let scopes = this.options._super ? [ this.options, this.options._super ] : [ this.options ]
