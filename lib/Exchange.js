...
weave.Exchange = class Exchange extends EventEmitter {
  ...
  behavior( name ) {
    ...
    if ( name === 'location' && typeof behavior === 'string' ) behavior = behavior.replace( /^~/, os.homedir() )
  ...
  status( status ) {
    ...
    if ( this.state > HEAD ) return garden.error( `Cannot write status ${status} to HTTP stream, already wrote ${this._WRITTEN_STATUS}!` )
  ...
  _clearHead() {
    ...
    let headers = this._WRITTEN_HEADERS
  ...
  write( content, encoding ) {
    ...
    if ( typeof content !== 'string' && !Buffer.isBuffer( content ) ) return garden.typeerror( 'Content must be a string or buffer' )
